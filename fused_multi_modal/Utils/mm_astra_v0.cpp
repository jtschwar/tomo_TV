...
// SIRT Reconstruction.
Vec mm_astra::SIRT(const Vec &inVol, int s, int nIter)
{ 
    Vec tmpHAADF(sigma.rows());
    Vec updateSIRT(sigma.rows());
    ...
    memcpy(&updateSIRT(0), vol->getData(), sizeof(float)*Ny*Nz);
    if (gamma == 1) {updateHAADF = sigma.transpose() * (updateSIRT - tmpHAADF); }
    else {
        spXXmatrix.diagonal().array() = inVol.array().pow(gamma - 1); 
        updateHAADF = gamma * spXXmatrix * (Vec (sigma.transpose() * (updateSIRT - tmpHAADF)) ); }
...
tuple<float,float> mm_astra::SIRT_data_fusion(float lambdaHAADF, float lambdaCHEM, int nIter)
{
    float costHAADF = 0; float costCHEM = 0; 
    ...
    for (int s=0; s < Nslice; s++) {
        ...
        // cout << "updateCHEM: " << updateCHEM.maxCoeff() << ", updateHAADF: " << updateHAADF.maxCoeff() << endl;
...
// SIRT Reconstruction.
Vec mm_astra::SART(const Vec &inVol, int s, int nIter)
{ 
    Vec tmpHAADF(sigma.rows());
    Vec updateSIRT(sigma.rows());
